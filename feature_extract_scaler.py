...
def extract_scaler_feat_from_tag(tag):
...
def extract_scaler_features(sector):
    for cam,ccd in np.ndindex((4,4)):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executer:
            results = executer.map(extract_scaler_feat_from_tag,tags)
            Data = []
            for i,feat in enumerate(results):
                ...
                if feat is not None and np.all(np.isfinite(feat)) and not np.any(np.isnan(feat)):
                    Data.append(feat)
            Data = np.array(Data)
            with open(f'Features\\features{sector}_{cam}_{ccd}_s.txt', 'w') as file:
...
if __name__ == '__main__':
    ...
    extract_scaler_features(36)
    extract_scaler_features(38)